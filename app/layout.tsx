  ...
  return (
    <html lang="en">
      <body className="min-h-screen antialiased">
        <header className="border-b border-teal-900/20 bg-teal-900 text-white shadow-sm">
            ...
            <nav className="flex flex-wrap gap-2 text-sm">
              {navItems.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="rounded-md px-3 py-2 text-sm text-teal-50 transition-colors hover:bg-white/10 hover:text-white"
                >
                  {item.label}
                </Link>
              ))}
